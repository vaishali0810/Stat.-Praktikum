...
ggplot(data = gather(dfultimate, variable, value, M.A15.34.Anteil), # Geschlecht: Mann
       mapping = aes(x = value)) +
  geom_density(aes()) +
  facet_wrap(facets = ~variable, scales = "free") + theme()
...
dfultimate <- dfultimate %>% mutate(M.A00.04.Anteil = M.A00.04/total_cases,
                      ...
                      M.Aunb.Anteil = M.Aunb./total_cases,
                     ...
                     F.Aunb.Anteil =F.Aunb./total_cases,
                    ...
                    Unb.Aunb.Anteil =Unb.Aunb./total_cases)
